...
# App icon. Generated by `swift scripts/make-icon.swift` and committed to the
# repo so contributors get a consistent build without re-running the script.
if [[ -f Resources/AppIcon.icns ]]; then
    cp Resources/AppIcon.icns "${APP_BUNDLE}/Contents/Resources/AppIcon.icns"
else
    echo "    warning: Resources/AppIcon.icns not found — built .app will use the generic Mac icon"
    echo "    fix: run \`swift scripts/make-icon.swift\` to regenerate"
fi
